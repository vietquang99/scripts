// ==UserScript==
// @name         Cursor Form Helper
// @namespace    http://tampermonkey.net/
// @version      1.5
// @description  Hiển thị form hỗ trợ trên cursor.sh
// @author       You
...
(function () {
    ...
    var IGNORE_DOMAINS = ['idebattisti.com', 'g2italia.net','rvcisp.com','dj-fs.com','ocramnaig.com'];
    ...
    async function fetchRandomDomains() {
        try {
            var response = await fetch('https://tinyhost.shop/api/random-domains/?limit=3');
            if (!response.ok) {
    ...
    async function getValidDomain() {
        ...
        while (attempts < maxAttempts) {
            ...
            // Lọc các domain không thuộc ignore list
    ...
    // Hàm click button Tiếp tục (Cursor)
    function clickContinueButton() {
        setTimeout(function () {
            var continueBtn = document.querySelector('button[name="intent"][value="sign-up"]');

            if (continueBtn && !continueBtn.disabled) {
                continueBtn.click();
                console.log('✅ Đã click button Tiếp tục');
            }
            else {
                console.log('⚠️ Button Tiếp tục đang disabled hoặc không tìm thấy');
            }
        }, 300);
    }
    ...
    formContainer.id = 'cursor-helper-form';
    formContainer.innerHTML =
        '<div id="wh-form-wrapper">' +
        '<div id="wh-header">' +
        '<span>🖱️ Cursor Helper</span>' +
    ...
    // Fill Random Info (Cursor - sử dụng class thay vì ID)
    document.getElementById('wh-fill-random').addEventListener('click', function () {
        ...
        // Tìm các input có class "rt-reset rt-TextFieldInput"
        var textInputs = document.querySelectorAll('input.rt-reset.rt-TextFieldInput');

        if (textInputs.length < 3) {
            showMessage('❌ Không tìm thấy đủ các input fields trên trang!', 'error');
            return;
        }

        // Giả sử thứ tự: Tên, Họ, Email
        var firstNameInput = textInputs[0];
        var lastNameInput = textInputs[1];
        var emailInput = textInputs[2];
        ...
        showMessage('✅ Đã điền thông tin thành công!', 'success');
    });

    // Fill Password (Cursor - chỉ có 1 input password, không có confirm)
    document.getElementById('wh-fill-password').addEventListener('click', function () {
        // Tìm input password có class "rt-reset rt-TextFieldInput"
        var passwordInput = document.querySelector('input[type="password"].rt-reset.rt-TextFieldInput');

        if (!passwordInput) {
            showMessage('❌ Không tìm thấy input password trên trang!', 'error');
            return;
        }
        ...
        showMessage('✅ Đã điền password thành công!', 'success');
    });
    ...
    async function fetchOTPFromEmail(domain, username) {
        try {
            var response = await fetch('https://tinyhost.shop/api/email/' + domain + '/' + username + '/?page=1&limit=20');
            if (!response.ok) {
                throw new Error('API request failed');
            }
            var data = await response.json();
    ...
    // Hàm extract OTP từ body email (Cursor)
    function extractOTPFromBody(body) {
        // Tìm mã OTP 6 số trong body
    ...
    // Fill OTP vào các input và tự động submit (Cursor)
    document.getElementById('wh-fill-otp').addEventListener('click', async function () {
        ...
        try {
            ...
            // Tìm email có subject chứa "Xác minh địa chỉ email của bạn"
            var verifyEmail = null;
            for (var i = 0; i < emails.length; i++) {
                if (emails[i].subject && emails[i].subject.indexOf('Xác minh địa chỉ email của bạn') !== -1) {
            ...
            if (!verifyEmail) {
                showMessage('⚠️ Không tìm thấy email xác thực Cursor!', 'warning');
                return;
            }

            // Extract OTP từ body
            var otp = extractOTPFromBody(verifyEmail.body);
            ...
            // Tìm các input OTP trên trang (Cursor sử dụng class ak-OtpInput)
            var otpInputs = document.querySelectorAll('.ak-OtpInput input.rt-TextFieldInput');
            ...
            showMessage('✅ Đã điền OTP thành công! Trang sẽ tự động chuyển hướng.', 'success');
    ...
    console.log('✅ Cursor Helper Form đã được tải thành công!');
})();
